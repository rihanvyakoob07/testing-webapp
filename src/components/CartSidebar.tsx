...
const CartSidebar = ({
  ...
  // Filter out invalid items (price <= 0) and calculate total
  const validItems = cartItems.filter((item) => item.price > 0);
  const total = validItems.reduce((sum, item) => sum + item.price * item.quantity, 0);

  // Handle edge case where there are no valid items
        ...
        <SheetContent className="flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-4">
            <div className="flex h-full items-center justify-center text-center">
              <p className="text-muted-foreground">Your cart is empty or contains invalid items.</p>
            </div>
          </div>
        </SheetContent>
      </Sheet>
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {validItems.length === 0 ? (
            <div className="flex h-full items-center justify-center text-center">
              <p className="text-muted-foreground">Your cart contains no valid items.</p>
            </div>
          ) : (
